...
const saveMessage = (msg) => {
  ...
  const timestamp = trimmedMsg.messageTimestamp;
  const data = trimmedMsg;
  insertStmt.run(msgId, jid, timestamp, JSON.stringify(data));
};

const hasSeenMessage = (msg) => {
    getStmt.run(msg.key.id);
    const row = getStmt.get(msg.key.id);
    return !!row;
}
...
export { saveMessage, getMessage, hasSeenMessage };
