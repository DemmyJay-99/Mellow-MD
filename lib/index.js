...
async function setvar(variable, value) {
  ...
  const envContent = fs.readFileSync(envFilePath, "utf8").replaceAll(' ', '');
  const lines = envContent.split("\n");
  const varIndex = lines.findIndex(line => line.startsWith(`${variable}=`));
  if (varIndex !== -1) {
...
export { getGroupConfig, setGroupConfig, getWarns, setWarns, readData, writeData, setvar, isGroup, isJid, isPrivate };