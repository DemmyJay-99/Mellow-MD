...
import { saveMessage, getMessage } from "./db.js";
...
const handleMessageRevocation = async (sock, message) => {
  ...
  const originalMessage = await getMessage(msgID);
  if (!originalMessage) {
    console.log(`Original message with ID ${msgID} not found in the database.`);
    return;
  }
  const pushname = originalMessage?.pushName || "Unknown";
  const chatID = originalMessage?.key?.remoteJid || "Unknown";
  const senderID = originalMessage?.key?.participant || originalMessage?.key?.remoteJid || "Unknown";
  const content = originalMessage?.message?.conversation || originalMessage?.message?.extendedTextMessage?.text || originalMessage?.message?.imageMessage?.caption || originalMessage?.message?.videoMessage?.caption || "No content available";
  console.log(`Message with ID ${msgID} has been revoked.`);
  const notificationText = `*Message Revoked*\n\n*Sender:* ${pushname}\n*Chat ID:* ${chatID}\n*Sender ID:* ${senderID}\n*Content:* ${content}`;
